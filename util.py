...
def max_flow(bandwidth, src_dict, dst_dict):
    ...
    flow_matrix = [
        [0 for col in range(len(bandwidth))] for row in range(len(bandwidth))
    ]
    for i in range(len(bandwidth)):
...
def add_instance(
    ...
    candidate_nodes_distance = sorted(
        sorted(
            candidate_nodes_distance, key=lambda n: node_list[n[0]].avail, reverse=True
        ),
        key=lambda n: n[1],
    )
    ...
    for v in candidate_nodes:
        ...
        if same_instance is not None:
            ...
            and v not in vector[nf_index + 1]
        ):
...
def get_route(bandwidth, request, request_placement):
    ...
    for k, segment in enumerate(flow_assignment):
        if k < len(flow_assignment) - 1:
            flow_matrix_segment = np.mat(
                max_flow(bandwidth, flow_assignment[k], flow_assignment[k + 1]),
                dtype="int32",
            )
